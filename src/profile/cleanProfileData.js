...
module.exports = (profile) => {
  ...
  if (!profile.profile) {
    const messageError =
      "LinkedIn website changed and scrapedin can't read basic data. Please report this issue at https://github.com/linkedtales/scrapedin/issues";
    logger.error('cleanMessageData', messageError, '');
    throw new Error(messageError);
  }

  if (profile.profile.connections) {
    profile.profile.connections = profile.profile.connections.replace(
      ' connections',
      '',
    );
  ...
  profile.recommendations.receivedCount = profile.recommendations.receivedCount.replace(
    /[^\d]/g,
    '',
  );
  profile.recommendations.givenCount = profile.recommendations.givenCount.replace(
    /[^\d]/g,
    '',
  );
